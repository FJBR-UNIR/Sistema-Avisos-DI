...
// Marzo 2024.
// LICENCIA: GNU LICENCIA GENERAL PÚBLICA Versión 2.

// PROGRAMA PARA LA ENTRADA
...
// BUZZER
 int pinbuzzer=6; //El buzzer está conectado en el pin 6

 //RFID
 int pinSDA=10; // conexión pin SDA/NSS en RFID
 int pinRST=9;  // conexión pin RST en RIFID 
...
// Se inicializan TODOS los dispositivos y se comprueba que están conectados  
Wire.begin();
RTC.begin();
...
lcd.begin(16,2);
lcd.setCursor(0,0);
lcd.write("ENTRADA ON - UNIR");
lcd.setCursor(0,1);
lcd.write("Fran. Berchez R.");
delay(2000);
lcd.clear();
tonomensaje();
mostrarmensajes("ENTRADA - NO OLVIDAR","ALARMA DE SEGURIDAD");  
delay(3000);
lcd.clear();
...
if (! RTC.isrunning()) {
    ...
    // Ajustar la hora del RTC a la hora de compilación y carga.
...
mostrarmensajes("ENTRADA - NO OLVIDAR","ALARMA DE SEGURIDAD");
myDFPlayer.playFolder(15, 40); 
delay(3000);
tonomensaje();  
luces();
mostrarmensajes("AL ENTRAR","DEJAR LLAVES Y CARTERA EN SU SITIO");
myDFPlayer.playFolder(15, 41); 
delay(3000); 
tonomensaje();
luces();
mostrarmensajes("ANTES DE SALIR","COGER LLAVES");
myDFPlayer.playFolder(15, 42); 
delay(3000); 
tonomensaje();
luces();
mostrarmensajes("ANTES DE SALIR","APAGAR LUCES Y COGER MÓVIL");
myDFPlayer.playFolder(15, 43); 
delay(3000); 
tonomensaje();
luces();
mostrarmensajes("ANTES DE SALIR","APAGAR AIRE ACONDICIONADO");
myDFPlayer.playFolder(15, 44); 
delay(3000); 
