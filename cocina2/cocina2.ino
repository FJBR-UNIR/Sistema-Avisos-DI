...
lcd.begin(16,2);
lcd.setCursor(0,0);
lcd.write("COCINA ON - UNIR");
lcd.setCursor(0,1);
lcd.write("Fran. Berchez R.");
delay(2000);
lcd.clear();
tonomensaje();
mostrarmensajes("DESPUES DE COCINAR","APAGAR VITROCERAMICA");  
delay(3000);
lcd.clear();
...
luces();
mostrarmensajes("DESPUES DE COCINAR","APAGAR GAS");
myDFPlayer.playFolder(15, 30); 
delay(3000);  
tonomensaje();
mostrarmensajes("DESPUES DE COCINAR","APAGAR VITROCERÁMICA");
myDFPlayer.playFolder(15, 31); 
delay(3000); 
tonomensaje();
mostrarmensajes("SI COCINAS","USA EL EXTRACTOR");
myDFPlayer.playFolder(15, 32); 
delay(3000); 
  if (now.hour()>=22 && now.hour()<=23){
    tonomensaje();
    luces();
    mostrarmensajes("DESPUÉS DE CENAR","PONER LAVAVAJILLAS");
